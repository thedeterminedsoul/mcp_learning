...
function safePath(inputPath) {
    const resolved = (0, path_1.join)(WORKING_DIR, inputPath);
    ...
    try {
        switch (name) {
            case "read-file": {
                ...
                const content = await fs_1.promises.readFile(safePath_resolved, "utf8");
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "write-file": {
                ...
                const dir = (0, path_1.dirname)(safePath_resolved);
                await fs_1.promises.mkdir(dir, { recursive: true });
                await fs_1.promises.writeFile(safePath_resolved, content, "utf8");
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "list-directory": {
                ...
                const items = await fs_1.promises.readdir(safePath_resolved);
                const itemDetails = await Promise.all(items.map(async (item) => {
                    const itemPath = (0, path_1.join)(safePath_resolved, item);
            ...
            case "create-directory": {
                const { path } = args;
                const safePath_resolved = safePath(path);
                await fs_1.promises.mkdir(safePath_resolved, { recursive: true });
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "delete-file": {
                ...
                const stats = await fs_1.promises.stat(safePath_resolved);
                if (stats.isDirectory()) {
                    await fs_1.promises.rmdir(safePath_resolved, { recursive: true });
                    return {
                        content: [{ type: "text", text: `Directory deleted: ${path}` }],
                    };
                }
                else {
                    await fs_1.promises.unlink(safePath_resolved);
                    return {
...
server.setRequestHandler(types_js_1.ListResourcesRequestSchema, async () => {
    ...
    try {
        const items = await fs_1.promises.readdir(WORKING_DIR);
        for (const item of items) {
            const itemPath = (0, path_1.join)(WORKING_DIR, item);
            const stats = await fs_1.promises.stat(itemPath);
            if (stats.isFile()) {
                ...
                if (textExtensions.includes(ext) || !ext) {
                    resources.push({
                        uri: `file://${(0, path_1.relative)(WORKING_DIR, itemPath)}`,
                        name: item,
